		...
		eHTTP.post(this.urls.fetchTable, filtro ,function(error, data){
			...
			datas.forEach(function(data){
				var html  = '<tr>';
					...
					html += '<td class="frequencia"><p class="frequencia bg-success text-light">' + data.presencas + '</p> | <p class="falta bg-warning">' + data.faltas + '</p></td><td><div class="action"><button class="btn btn-primary btn-small">Editar</button> | <button class="btn btn-danger btn-small">Desativar</button></div></td>';
					html += '</tr>';
					bodyTable.insertAdjacentHTML('beforeend',html);
			});
		});
